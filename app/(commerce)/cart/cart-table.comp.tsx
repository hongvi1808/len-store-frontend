...
export function CartTableComp(props: CartTableProps) {
    ...
    const { isLoading, data } = useQuery({
        queryKey: ['cart-table-customer', paginationModel, user.userId],
        queryFn: async () => cartApis.getList({ ...paginationModel, customerId: user.userId }),
        enabled: !!user.userId
    });
    // MUTATE
    const { mutate: mutateUpdateCartItem, isPending: pendingUpdate } = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['cart-table-customer', paginationModel, user.userId] });
        },
    });
    ...
    const onSelectedChange = (e: any) => {
        let selectItem;
        if (e.type === 'include') {
            const selectedData = (loggedIn ? data?.items : cartLocal).filter((row: any) => e.ids.has(row.id));
        ...
        if (e.type === 'exclude' && !e.ids.size) {
            selectItem = (loggedIn ? data?.items : cartLocal).map((i: any) => ({
                id: i.product.id, image: i.product.images?.[0],
                name: i.product.name, price: i.product.price,
                quantity: i.quantity, classify: i.classify
            }))
    ...
    const handleRemove = (item: any) => {
        ...
        else dispatch(updateLocalCart(item))
    }
        ...
        { field: 'price', headerName: 'Đơn giá', flex: 1, renderCell: (params) => <Box>{formatCurrency(params.row.product.price)}</Box> },
        {
            ...
            renderCell: (params) => {
                return <Stack direction="row" alignItems="center" spacing={2} paddingY={1}>
                    <IconButton loading={pendingUpdate}
