...
import { updateStatusOrder } from "@/base/store/slices/order.slice";
...
import { Badge, Box, Card, CardMedia, Divider, Stack, Typography } from "@mui/material";
...
export function OrderCustomerForm() {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        mutate({ ...data, products, totalPrice, paymentMethod })
        // TODO call api post
    }
    return (
            ...
            <Stack flex={1} spacing={2}>
                ...
                <Box id="order-customer-form"
                    ...
                    <TextFiledControlBase
                        ...
                        inputProps={{ placeholder: 'Tên*', required: true, }}
                    />
                    <TextFiledControlBase
                        name='phoneNumber'
                        getErrorMessage={validPhone}
                        inputProps={{ placeholder: 'Số điện thoại*', required: true, }}
                    />
                    <TextFiledControlBase
                        name='email'
                        getErrorMessage={(value) => validEmail(value, true)}
                        inputProps={{ placeholder: 'Email (tùy chọn)', }}
                    />
                    <TextFiledControlBase
                        name='address'
                        getErrorMessage={validRequire}
                        inputProps={{ placeholder: 'Địa chỉ*', required: true, }}
                    />
                    <TextFiledControlBase
                        name='note'
